...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?, ?, (SELECT id FROM Positions WHERE position = ?), ?, ?)", (id, name, position, birthyear, nationality))
...
def position_birth_search(position, age, cur, conn):
    birth_year = 2023 - age
    cur.execute("SELECT Players.name, Positions.position, Players.birthyear FROM Players JOIN Positions ON Players.position_id = Positions.id WHERE Positions.position = ? AND Players.birthyear > ?", (position, birth_year))
    results = cur.fetchall()
    return results
...
def make_winners_table(data, cur, conn):
    ...
    for item in data['seasons']:
        id = item['id']
        if item['winner'] != 'null' and item['winner'] != None:
            winner = item['winner']['name']
            cur.execute("INSERT OR IGNORE INTO Winners (id, name) VALUES (?, ?)", (id, winner))
    conn.commit()
